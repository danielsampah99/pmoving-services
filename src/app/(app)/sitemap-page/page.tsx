...
export default async function SitemapPage() {
	...
	const [blogs, serviceAreas] = await Promise.all([await getBlogs(), await getServiceAreas()])
	...
	return (
		<Fragment>
			...
			<main className="p-4 isolate lg:px-8 mx-auto relative bg-white">
				<h1 className="py-10 text-gray-800 inline-flex items-center w-full my-4 text-2xl font-bold">Sitemap</h1>
				...
				<div className="">
					...
					<ol className="mt-2 text-gray-700 font-normal grid grid-cols-2 text-sm gap-y-0.5">
						{Array.isArray(services) && services.map((serv, servIdx) => (<Link key={serv.id} href={serv.href} className="block">
							{servIdx}. {serv.title}
						</Link>))}
					</ol>
				</div>
				...
				<div>
					<div className="inline-flex items-center w-full my-4 text-2xl font-bold"><span className="underline">Blogs</span>
						<BlogIcon className="size-8 ml-4" />
					</div>

					<ol className="mt-2 text-gray-700 font-normal grid text-sm gap-y-0.5">
						{Array.isArray(blogs.docs) && blogs.docs.map((doc, docIdx) => (
							<Link key={doc.id} href={`/blogs/${doc.slug}`} className="block capitalize hover:underline">
				...
				<div>
					<div className="inline-flex items-center w-full my-4 text-2xl font-bold"><span className="underline">Service Areas</span>
						<GoogleMapsIcon className="size-8 ml-4" />
					</div>

					<ol className="mt-2 text-gray-700 font-normal grid text-sm gap-y-0.5">
						{Array.isArray(serviceAreas.docs) && serviceAreas.docs.map((sa, saIdx) => (
							<Link key={sa.id} href={`/service-areas/${sa.slug}`} className="block capitalize hover:underline">
