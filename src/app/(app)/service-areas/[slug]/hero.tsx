			...
			<div className="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-2 lg:items-center lg:gap-y-0">
				<div className="lg:row-start-2 lg:max-w-md">
					<div className="flex flex-col sm:flex-row gap-y-3 sm:gap-x-5 sm:items-baseline">
						{props.mapUrl && (<Link href={props.mapUrl} target="_blank" title={`Find ${props.title} on the map`}>
			...
			{props.stats.map((area) => (
				<div key={area.name} className="relative">
					<dt className="ml-9 inline-block font-semibold text-gray-700">
						<area.icon aria-hidden="true" className="absolute left-1 top-1 size-5 lg:size-6 stroke-moving-gray fill-moving-yellow" />
						{area.name}
					</dt>{' '}
