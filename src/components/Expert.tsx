      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <motion.div variants={fadeInUp} className="mx-auto max-w-2xl lg:mx-0">
          ...
          <p className="mt-6 text-lg font-medium text-background sm:text-xl">
            At Premium Moving Services, we ensure that all employees receive the same expert training, providing 
            consistency and excellence across all jobs. Every new hire undergoes a rigorous training program to learn
            safe and efficient packing and moving techniques in various situations.
          </p>
        </motion.div>

        {/* Links */}
        <motion.div variants={fadeInUp} className="mx-auto mt-10 max-w-2xl lg:mx-0 lg:max-w-none">
          <div className="grid grid-cols-1 gap-x-8 gap-y-6 text-base font-semibold text-background sm:grid-cols-2 md:flex lg:gap-x-10">
            {links.map((link) => (
              <motion.a key={link.name} href={link.href} whileHover={{ scale: 1.1 }}>
        ...
        {/* Stats Section */}
        <motion.dl variants={staggerContainer} className="mt-16 grid grid-cols-1 gap-8 sm:mt-20 sm:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat) => (
            <motion.div key={stat.name} variants={fadeInUp} className="flex flex-col-reverse gap-1">
              <dt className="text-base text-background">{stat.name}</dt>
              <dd className="text-4xl font-semibold tracking-tight text-background">{stat.value}</dd>
            </motion.div>
          ))}
        </motion.dl>
      </div>
