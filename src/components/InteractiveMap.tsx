...
import { MapCity, minnesotaMapCities, states, wisconsinMapCities } from "@/map-data"
...
export const InteractiveMap: FC = () => {
	...
	const [activeState, setActiveState] = useState<'all' | 'minnesota' | 'wisconsin'>('all')
	const [hoveredCity, setHoveredCity] = useState<string | null>(null)
	const [selectedCity, setSelectedCity] = useState<string | null>(null)
	const [mapStyleIndex, setMapStyleIndex] = useState(0)
	const [mapCenter, setMapCenter] = useState<LatLngExpression>([46, -94]) // center of minnesota
	const [mapZoom, setMapZoom] = useState(8)
	const [cities, setCities] = useState<MapCity[]>([...minnesotaMapCities, ...wisconsinMapCities])
		...
		<section className="py-12 bg-slate-50">
			<div className="container mx-auto px-4">
				<div className="text-center mb-8">
					<h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-3">Our Service Areas</h2>
					<p className="text-lg text-slate-600 max-w-2xl mx-auto">
						We provide moving services throughout Minnesota and Wisconsin. Explore our service areas on the interactive
						map.
					</p>

					<LocationControl states={states} currentState={activeState} onStateChange={handleStateClick} />
				</div>
