...
const ImageCard: React.FC<ImageCardProps> = ({ image, onClick, index }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-lg aspect-auto" onClick={onClick}>
        {/* Image */}
        <img 
          src={image.thumbnail} 
          alt={image.alt}
      ...
      <div className="p-3 bg-white">
        <h3 className="font-medium text-moving-dark text-sm mb-1">{image.title}</h3>
        <p className="text-moving-gray text-xs">{trimDescription(image.description)}</p>
      </div>
    </div>
  );
};
